...
Phase 2 keeps all extracted formats in the session and layers `FavoriteMatcher` presentation over them. SQLite stores flexible ordered rules, per-user delivery/detail settings, scoped collection/filter drafts, exact Telegram-file cache entries and coalesced-job subscribers. Schema v10 adds validated operational overrides consumed by `RuntimeSettingsService`; environment values remain bootstrap defaults. `selection.py` is the single admission path for delivery preflight, cache reuse, exact active-job coalescing, queue limits, and job creation.

Collections and multi-URL batches store only opaque item IDs in callback data. A multi-link message creates one persisted batch session and one edited status card while bounded extraction leases analyze children independently. An item reuses embedded genuine formats or is extracted on explicit selection. Auxiliary source assets use controlled streaming; video/audio jobs retain exact fingerprints and use the same scheduler/no-transcoding path.
